...
import { Target, Hammer, Lightbulb, PenTool, Cloud, Users, Briefcase, ShieldCheck, Leaf } from 'lucide-react';
...
const TagsSection = () => {
  ...
  return (
    <div className="w-full bg-[#f2f1eb] py-12 overflow-hidden relative">
      ...
      <div className="flex flex-col gap-4">
        ...
        >
          {[...tags, ...tags].map((tag, idx) => (
            <div key={`row1-${idx}`} className="flex items-center gap-2 bg-white/60 backdrop-blur-sm px-6 py-3 rounded-full border border-gray-200/50 text-gray-700 font-medium text-sm">
        ...
        >
          {[...tags.reverse(), ...tags].map((tag, idx) => (
            <div key={`row2-${idx}`} className="flex items-center gap-2 bg-white/60 backdrop-blur-sm px-6 py-3 rounded-full border border-gray-200/50 text-gray-700 font-medium text-sm">
              <tag.icon className="w-4 h-4 text-gray-500" />
              {tag.text}
            </div>
          ))}
