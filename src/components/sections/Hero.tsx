...
const Hero = () => {
  return (
    <section id="hero" className="w-full max-w-[1536px] mx-auto min-h-[776px] px-[50px] pt-[20px] pb-[100px] bg-[#fafafa] relative z-[2] overflow-hidden flex justify-center items-center box-border transition-all">
      <div className="relative overflow-hidden max-w-[1436px] w-full min-h-[656px] flex items-center box-border z-[4] transition-all p-8 lg:p-20 rounded-[36px]">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center w-full">
          ...
          >
            ...
            <div className="flex items-center gap-4 pt-6 border-t border-black/15 w-full max-w-[400px]">
              <div className="flex gap-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} className="w-5 h-5 fill-[#0f291e] text-[#0f291e]" />
                ))}
              </div>
              <div className="w-px h-5 bg-black/15"></div>
              <span className="text-gray-900 font-semibold text-sm">Rated by loving Clients</span>
            </div>
          </motion.div>
          ...
          <motion.div 
            ...
            className="lg:col-span-6 flex flex-col items-end gap-10"
          >
            <div className="relative flex gap-6 w-full lg:w-[576px] lg:h-[438.667px] bg-transparent rounded-[40px] p-6 lg:p-8 box-border flex-shrink-0">
              <img 
            ...
            <div className="flex items-center gap-4">
              <div className="flex -space-x-2">
                <img src="https://i.pravatar.cc/100?img=11" alt="Client" className="w-10 h-10 rounded-full border-2 border-[#ecebe4] object-cover" />
                <img src="https://i.pravatar.cc/100?img=12" alt="Client" className="w-10 h-10 rounded-full border-2 border-[#ecebe4] object-cover" />
                <img src="https://i.pravatar.cc/100?img=13" alt="Client" className="w-10 h-10 rounded-full border-2 border-[#ecebe4] object-cover" />
                <img src="https://i.pravatar.cc/100?img=14" alt="Client" className="w-10 h-10 rounded-full border-2 border-[#ecebe4] object-cover" />
              </div>
              <span className="text-sm font-bold text-gray-900">Join 1,000 + other awesome clients</span>
            </div>
          </motion.div>
        </div>
      </div>
